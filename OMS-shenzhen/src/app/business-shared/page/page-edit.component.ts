...
export class PageEditComponent implements OnInit, OnDestroy {
  ...
  getDetail () {
    ...
    this.pageBusinessService.pageDetail(param, function(data){
      ...
      let strA = data.bgImage !== null ? (data.bgImage !== '' ? JSON.parse(localStorage.userInfo).imageUrl + '/HD/' + data.bgImage + "?random=" + Math.random() : "") : '';
      let strB = data.bgImage !== null ? (data.bgImage !== '' ? JSON.parse(localStorage.userInfo).imageUrl + '/SD/' + data.bgImage + "?random=" + Math.random() : "") : '';
      let strC = data.template !== null ? (data.template !== '' ? JSON.parse(localStorage.userInfo).imageUrl + '/HD/' + data.template + "?random=" + Math.random() : '') : '';
      that.imgSrcA = strA;
      that.imgSrcB = strB;
      that.imgSrcC = strC;
      that.getpri();
