...
@Component({
  selector: 'c-inOutCharge-list',
  templateUrl: './inOutCharge-list.component.html',
  styleUrls: ['./inOutCharge-list.component.scss']
})
export class InOutChargeListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.appService.titleEventEmitter.emit("内外计费编码关联");
  }
  ...
  getList() {
    ...
    let param = {
      ...
      ttpProductCode: this.ttpProductCode,
      productCode:    this.productCode,
    }
    this.odsysBusinessService.productOwnTipList(param, function(data){
  ...
  save(id) {
    let that = this;
    this.codeSaveModal = this.ngbModalService.open(InOutChargeSaveComponent, {size: 'lg'});
    if (id) {
  ...
  del(id) {
    ...
    if (id !== '') {
      param.push({
        ...
        ttpProductCode: id.ttpProductCode
      });
    } else {
      param = this.getIds();
    }
    ...
    this.modalService.confirm(confirmCfg).then((result) => {
      that.odsysBusinessService.productOwnTipDel ({primaryKeys: JSON.stringify(param)}, function(data){
        that.getList();
    ...
    this.dataList.forEach(i => {
      if (i.checkState) {
        ids.push({
          ...
          ttpProductCode: i.ttpProductCode
        });
      }
    });
    return ids;
