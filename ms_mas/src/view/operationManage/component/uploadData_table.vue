<template>
  <div>
    ...
    <el-table
      ...
      <el-table-column
        prop="unicomRetainedUsers"
        sortable
        min-width="83"
        label="留存用户数">
      </el-table-column>
      <el-table-column
        prop="unicomSuccessOrders"
        sortable
        min-width="83"
        label="订购成功数">
      </el-table-column>
      <el-table-column
        prop="unicomOffOrders"
        sortable
        min-width="83"
        label="退订用户数">
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      initTable(){
        ...
        //计算适应高度
        that.$ajax({
          url: "/ms_bas/upload_data/queryAll.do",
          method: "post",
          data: {},
          ...
          error: function (error) {
            that.$message.error('局方录入数据页面数据请求失败，请刷新局方录入数据页面');
          }
        });
      },
      ...
      add(){
        let that = this;
        that.$emit("uploadEdit",{type:'add',data:''})
      },
      edit(data){
        let that = this;
        that.$emit("uploadEdit",{type:'edit',data:data})
      },
      del(data) {
        let that = this;
        that.$emit("uploadDel", {data: data});
      }
    }
  }
</script>
