<template>
  <div class="userManage">
    ...
    <v-search
      :pagenum="pagenum"
      :refresh="refresh"
      @searching="tableLoading = true"
      @searchData="getSearchResult($event)"></v-search>
    <!--table-->
    <v-table
      v-loading="tableLoading"
      :table-data="tableData"
      :pass-word-change="pass_word_change"
    ...
    <el-dialog
      ...
      <el-form :model="dialogForm" status-icon :rules="rules2" ref="dialogForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="组织机构" v-if="dialogForm.departname != undefined" prop="orgId">
          <span class="formItem">
            <el-select
  ...
  export default {
    ...
    data () {
      ...
      const validate = {
        ...
        userName (rule, value, callback) {
          if ($vue.dialogFormAddnew === true) {
              callback();
          } else if (value === '') {
            callback(new Error('请填写用户名'));
          } else if(value.length > 15 || value.length < 2) {
            callback(new Error('用户名长度为2~15个字符'));
          } else {
            $vue.$ajax({
      ...
      return {
        ...
        rules2: {
          realName: [
            { required: true, validator: validate.realName, trigger: "blur" }
          ],
          userName: [
          ...
          orgId: [
            { required: true, validator: validate.orgId, trigger: "change" }
          ],
          userKey: [
            { required: true, validator: validate.userKey, trigger: "change" }
          ],
        },
    ...
    methods: {
        ...
        that.$ajax({
          ...
          success: function (res) {
            if (res.data.success) {
              ...
              that.pass_word_change = !that.pass_word_change;
            } else {
              that.$message({
                type: "warning",
      ...
      getSearchResult (obj) {
        ...
        that.tableData = {rows: obj.rows, total: obj.total, current: obj.current};
      },
      getDatagridList () {
        let that = this;
      ...
      editUserName (obj) {
        ...
        that.$ajax({
          url: url + "?userid=" + that.dialogForm.id,
