...
<script>
  ...
  export default {
    ...
    data () {
      return {
        url: {
          list: "/ms_bas/dianboRate/getTableData.do",
          img: "/ms_bas/dianboRate/getImgData.do",
          listCount: "/ms_bas/dianboRate/getTableData",
          listDetail: "/ms_bas/dianboRate/getTableDataDetail",
        },
        search_form: {
          startDate: "",
          endDate: "",
    ...
    methods: {
      ...
      search () {
        ...
        that.$ajax({//图形数据
          ...
          error: function (err) {
            that.$message.error('点播率分析报表页面数据请求失败，请刷新点播率分析报表页面');
            that.loading = false;
            that.$emit("loadingImg", false);
          }
        })
      ...
      toPostImgData(){
        ...
        let imgdata ={};
        if (that.imgData.compareLists !== undefined && that.imgData.data !== undefined){
          let name =['当前','对比'];
          ...
          that.imgData.data.forEach((item,index)=>{
            ...
            for ( let key in item){
              if (that.searchType.code === key){
                now.push(Number(item[key]));
              }
            }
          });
          that.imgData.compareLists.forEach((item,index)=>{
            for ( let key in item){
              if (that.searchType.code === key){
                before.push(Number(item[key]));
              }
            }
          });
          let data=[];
