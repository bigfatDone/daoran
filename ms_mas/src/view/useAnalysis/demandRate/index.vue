  ...
  export default {
    ...
    data () {
      return {
        ...
        selectbars:[ //切换按钮
          {text:'用户点播率',tooltip:'存量访问用户数/存量用户数',status:true,code:'userOnDemandRate',chartType:'spline',yPlus:'%'},
          {text:'用户人均点播时长',tooltip:'存量点播用户数/存量用户数',status:false,code:'userOnDemandTime',chartType:'spline',yPlus:'分'},
        ],
        searchType:{},
        imgDataState:false,
        loadingTable:true,
    ...
    methods: {
      ...
      downloadTable(){
        let that = this;
        let url = "/ms_bas/dianboRate/toExcel.do?startDate="+that.search_form.startDate +"&endDate="+that.search_form.endDate
