  ...
  export default {
    ...
    data () {
      return {
        ...
        selectbars:[ //切换按钮
          {text:'存量用户访问活跃率',tooltip:'存量访问用户数/存量用户数',status:true,code:'iStockAAvg',chartType:'spline',yPlus:'%'},
          {text:'存量用户点播活跃率',tooltip:'存量点播用户数/存量用户数',status:false,code:'iStockPAvg',chartType:'spline',yPlus:'%'},
          {text:'存量用户人均点播时长',tooltip:'存量用户点播时长/存量点播用户数',status:false,code:'iStockPAvgTime',chartType:'spline',yPlus:'分'},
        ],
        searchType:{},
        imgDataState:false,
        loadingTable:true,
    ...
    methods: {
      ...
      downloadTable(){
        let that = this;
        let url = "/ms_bas/activityRateReport/toExcel.do?startDate="+that.search_form.startDate +"&endDate="+that.search_form.endDate
