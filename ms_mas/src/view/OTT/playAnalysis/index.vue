...
//        that.initImg();
      ...
      initTable() {
         let that = this;
        let url = '/ms_bas/playAnalysis/getPlayTable';
        that.$ajax({
          url: url,
          data: {
          ...
          error: function (err) {
            that.$message.error('点播分析报表新页面数据请求失败，请刷新点播分析报表新页面');
          }
        });
      },
      // 获取hidechart数据
      ...
      downloadTable(){ // 汇总下载
        let that = this;
        let url = "/ms_bas/playAnalysis/toExcel?startDate="+that.search_form.startDate +"&endDate="+ that.search_form.endDate
