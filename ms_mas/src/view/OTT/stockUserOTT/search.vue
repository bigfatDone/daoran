...
<script>
  ...
  export default {
    ...
    props:['searchType','selectbars', 'tableType'],
    components: {
      vDate,vSelect
    },
    data () {
      return {
        treeData: [],
        url: {
          table: "/ms_bas/ott/user/stock/getTable",
    ...
    watch:{
      ...
      tableType(ev) {
        if (ev === 'new') {
          this.url.table = "/ms_bas/ott/user/stock/getTable";
        } else {
          this.url.table = "/ms_bas/ott/user/stock/getAccessTable";
        }
        this.search();
      }
    },
