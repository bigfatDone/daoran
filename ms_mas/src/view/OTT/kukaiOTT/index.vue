...
//        that.initImg();
      ...
      initTable() {
         let that = this;
        let url = '/ms_bas/ott/kukai/getTable';
        that.$ajax({
          url: url,
          data: {
          ...
          error: function (err) {
            that.$message.error('酷开渠道报表页面数据请求失败，请刷新酷开渠道报表页面');
          }
        });
      },
      // 获取hidechart数据
      ...
      downloadTable(){ // 汇总下载
        let that = this;
        let url = "/ms_bas/ott/kukai/toExcel?startDate="+that.search_form.startDate +"&endDate="+ that.search_form.endDate
