...
//          {id: 'nearsixty', text: '按季度', status: false, visiable: this.buttons.indexOf("nearsixty") >= 0},
        ...
        pickerOptions2: {// 日期一个星期之前
          disabledDate: (time) => {
            ...
            if (this.endDate != "") {
              return time.getTime() > Date.now()-3600 * 1000 * day || time.getTime() >  new Date(this.endDate);
            } else {
              return time.getTime() > Date.now()-3600 * 1000 * day;
            }
//            return time.getTime() > Date.now()-3600 * 1000 * 24 * day;
        ...
        pickerOptions4: {// 日期一个星期之前
          disabledDate: (time) => {
            ...
            return time.getTime() > Date.now()-3600 * 1000 * 24 * day || time.getTime() < new Date(this.startDate).getTime()-3600 * 1000 * 24;
          },
          firstDayOfWeek: 1
        },
      }
...
//        this.endDate = this.$dateFormat(startWeek + 3600 * 1000 * 24 * 5,"yyyy-MM-dd")
      ...
      getWeekEnd() {// 获取周结束时间
        let endWeek = new Date(this.endDate).getTime();
        this.endDate = this.$dateFormat(endWeek + 3600 * 1000 * 24 * 5,"yyyy-MM-dd")
        let date = {
          dateStart:this.startDate,
          dateEnd:this.endDate,
          dateType:'week'
