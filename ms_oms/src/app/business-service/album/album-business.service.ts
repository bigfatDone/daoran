  ...
  constructor(private httpService: HttpService, private router: Router) {
    ...
    // if (this.windowUrl.indexOf("/program/") >= 0 ) {
      this.api = {
        ...
        delAlbumImage: '/busi/album/delAlbumImage',
      };
    // } else if (this.windowUrl.indexOf("draw/programList") >= 0 ) {
    }
  // }
  delAlbumImage(param, callback) {
    this.httpService.postFormData(this.api.delAlbumImage, param, function (successful, data, res) {
      if (successful) {
        callback(data.obj);
      }
...
/*  saveAlbumPost(param, callback) {
  ...
  saveAlbumPost(param, callback) {
    this.httpService.postFormFile(this.api.albumSave, param, function (successful, data, res) {
      if (successful) {
        callback( );
      }
    }, function (successful, msg, err) {
