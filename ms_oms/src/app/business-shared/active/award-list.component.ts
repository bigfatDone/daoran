...
export class AwardListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.appService.titleEventEmitter.emit("奖品信息");
  }
  ...
  getList() {
    ...
    this.activeBusinessService.getAwardList(param, function(data){
  ...
  save(id) {
    let that = this;
    this.activeSaveModal = this.ngbModalService.open(AwardSaveComponent, {size: 'lg'});
    if (id) {
  ...
  del(id) {
    ...
    this.modalService.confirm(confirmCfg).then((result) => {
      that.activeBusinessService.delAward (param, function(data){
        that.getList();
