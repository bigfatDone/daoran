...
@Component({
  selector: 'c-orderInfo-list',
  templateUrl: './orderInfo-list.component.html',
})
export class OrderInfoListComponent implements OnInit, OnDestroy {
  model: NgbDateStruct;

  orderRuslt: Array<any>= [{type: 0, value: '失败'}, {type: 1, value: '成功'}];
  dataList: Array<any>= [];
  options: any = {
    rows: 20,
    page: 1,     // 当前页
  ...
  constructor(
    ...
    this.appService.titleEventEmitter.emit("活动订购记录");
  }
  ...
  getList() {
    ...
    this.activeBusinessService.orderInfoList(param, function(data){
