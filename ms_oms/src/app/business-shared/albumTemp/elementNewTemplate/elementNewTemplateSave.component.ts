...
@Component({
  selector: 'c-elementNewTemplateSave-save',
  templateUrl: './elementNewTemplateSave.component.html',
  styleUrls: ['./elementNewTemplateSave.component.scss'],
})
export class ElementNewTemplateSave implements OnInit, OnDestroy {
  ...
  ok(): void {
    ...
    if (this.saveEleForm.valid) {
      ...
      this.albumTempBusinessService.saveTemplateElement(this.newFile, function(data){
        const toastCfg = new ToastConfig(ToastType.SUCCESS, '', data.msg, 2000);
        that.toastService.toast(toastCfg);
        if (that.modalData.type === 'add') {
          data.obj.isNewElement = 'yes';
        }
