...
@Component({
  selector: 'c-inCharge-list',
  templateUrl: './inCharge-list.component.html',
  styleUrls: ['./inCharge-list.component.scss']
})
export class InChargeListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.appService.titleEventEmitter.emit("内部计费产品配置");
  }
  ...
  save(id) {
    let that = this;
    this.codeSaveModal = this.ngbModalService.open(InChargeSaveComponent, {size: 'lg'});
    if (id) {
  ...
  del(id) {
    ...
    this.modalService.confirm(confirmCfg).then((result) => {
      that.odsysBusinessService.ownProductDel (param, function(data){
        that.getList();
