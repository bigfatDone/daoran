...
export class ElementSaveComponent implements OnInit, OnDestroy {
  ...
  windowUrl: any;
  drawFlag = true; // 指当前页   若为false 手绘本进入
  modalData: any = ''; //  若为空  新增
  preData: any = '';
  ...
  ngOnInit() {
    ...
    if (!this.drawFlag) {
      ...
      // if (this.eleTypes.length < 14) {
      //   this.eleTypes.push({type: 'intelligence', name: '智能推荐'});
      // }
    }
  ...
  getIntelList() {
    ...
    let param: any;
        param = {
          resType: 'intelligence'
        };
    this.pageBusinessService.intelList(param, function(data){
      that.saveEleForm.patchValue({
        eleValue: data.obj.albumCode,
      });
      that.IntelImg(data.obj);
    });
  }
  // 智能推荐图片
  IntelImg(data) {
      ...
      if ( i === 0 ) {
        if ( !!data.image) {
          this.imgDataArray.push({
            sort: 1,
            file1: this.imgApi + data.image,
            file2: this.imgApiSD + data.image,
            desc: data.imageDes,
            delAtt: false,
          });
        } else {
        ...
        if ( !!data.imageSecond) {
          this.imgDataArray.push({
            sort: 2,
            file1: this.imgApi + data.imageSecond,
            file2: this.imgApiSD + data.imageSecond,
            desc: data.imageSecondDes,
            delAtt: false,
          });
        } else {
          if (!!data.imageThird) {
        ...
        if ( !!data.imageThird) {
          this.imgDataArray.push({
            sort: 3,
            file1: this.imgApi + data.imageThird,
            file2: this.imgApiSD + data.imageThird,
            desc: data.imageThirdDes,
            delAtt: false,
          });
        }
      }
    }
    this.newFile.delete( 'fileA', '');
    this.newFile.delete( 'fileD', '');
    this.newFile.delete( 'fileB', '');
    this.newFile.delete( 'fileE', '');
    this.newFile.delete( 'fileC', '');
    this.newFile.delete( 'fileF', '');
  }
  getTagList() {
    const that = this;
    const param = {};
  ...
  getDetail (callback) {
    ...
    if (!!this.modalData.preFlag) {
      ...
      this.pageBusinessService.elePreDetailPost(param, function(data){
        const date = data.startTimeStr;
        const enddate = data.endTimeStr;
        const xxx = date.split(' ')[0].split('-');
        const xxxEnd = enddate.split(' ')[0].split('-');
  ...
  getEleValData(id) {
    ...
    if (eleType === 'page') {
      ...
      param = {label: id, tagCategory: this.tagCategory};
    ...
    }else if (eleType === 'course') {
      param = {eleValue: id};
      this.pageBusinessService.getCourseListByEleValue(param, function(data){
        if (data.length > 0) {
          that.eleValData = data;
        } else {
          that.eleValData = [];
        }
      });
  ...
  eleTypeChange() {
    ...
    // 智能推荐
    if ( eleVal === 'intelligence') {
      that.getIntelList();
      that.ligentFlag = true;
    } else {
      if ( this.modalData !== '') {
        that.doImg();
      }
      that.ligentFlag = false;
    }
    ...
    } else if (valLigent.indexOf(eleVal) !== -1) {// 智能推荐
      this.resTypes.push({type: 4, name: '周点播量'}, {type: 5, name: '最新上线'});
    }

    // 元素属性
    const val4 = ['vlist', 'plist', 'res', 'album'];
    const val5 = ['Android', ];
    const val6 = ['intelligence', ];
  ...
  showThisEle() {
    ...
    this.showThisEleModal.result.then((result) => {
      ...
      if (this.saveEleForm.controls['eleType'].value === 'tag') {
        that.saveEleForm.get(['eleValue']).setValue(result.id);
        this.tagCategory = result.tag;
        that.getEleValData(result.id);
        that.tagDataList.forEach( i => {
  ...
  addImg() {
    const len = this.imgDataArray.length;
    this.imgDataArray.push({
      sort: len + 1,
      file1: '',
      file2: '',
  ...
  ok(type): void {
    ...
    if (this.modalData !== '' && this.relationList.length !== 0) { // 点击确认是否保存到历史
      this.showThisEleModal = this.ngbModalService.open(ElementConfirSaveComponent, {size: 'sm'});
      this.showThisEleModal.componentInstance.oData = {
        oData: this.relationList,
        eleType: this.saveEleForm.controls['eleType'].value,
  ...
  edit(callback): void {
    ...
    if (this.saveEleForm.valid) {
      ...
      const ET = this.saveEleForm.getRawValue().eleType; // 判断是否添加到历史运营元素
  ...
  toLink(e) {
    ...
    const ele = this.saveEleForm.getRawValue().eleType;
